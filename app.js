// ======================================================
// STORAGE TASKS - APP.JS (CENTERED FIX & STORAGE LOGIC)
// ======================================================
...
// === STORAGE CALCULATION (FITUR BARU) ===
...
async function checkSession() {
    ...
    try {
        currentUser = await account.get();
        nav('dashboardPage'); updateGreeting(); calculateStorageUsage(); loadFiles('root');
...
async function loadFiles(folderId) {
    ...
    // Perbarui Judul Folder secara Dinamis
    if(folderId === 'root') {
        updateGreeting(); 
    } else {
        headerTitle.innerHTML = `<button onclick="loadFiles('root')" class="btn-pill small" style="background:rgba(255,255,255,0.2); width:auto; padding:0 15px; margin-right:15px;"><i class="fa-solid fa-arrow-left"></i> Kembali</button> ${currentFolderName}`;
    }
    ...
    try {
        ...
        if(res.documents.length === 0) grid.innerHTML = `<p style="color:rgba(255,255,255,0.3); width:100%; text-align:center;">Folder Kosong</p>`;
...
function renderItem(doc) {
    ...
    // Logika Thumbnail Sesuai Jenis File
    let previewContent = '';
    const fileNama = (doc.nama || "Tanpa Nama").toLowerCase();
    
    if (isFolder) {
        previewContent = `<i class="icon fa-solid fa-folder"></i>`;
    } else if (fileNama.match(/\.(jpg|jpeg|png|gif|webp)$/)) {
    ...
    } else if (fileNama.match(/\.(mp4|mov)$/)) {
...
window.submitUploadFile = async () => {
    ...
    try {
        ...
        loadFiles(currentFolderId); calculateStorageUsage();
    } catch (e) { alert(e.message); } finally { hideLoading(); }
};
