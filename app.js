...
// ======================================================
// 3. LOGIKA OTENTIKASI (SOLUSI LOGIN ERROR)
// ======================================================
...
if (el('loginForm')) {
    el('loginForm').addEventListener('submit', async (e) => {
        ...
        try {
            // 1. Cek Login Username (Jika tidak ada '@')
            if (!inputId.includes('@')) {
                try {
                    ...
                    if (res.total > 0) {
                        inputId = res.documents[0].email; // Temukan email dari username
                    } else {
                        throw new Error("Username tidak ditemukan.");
                    }
                } catch (err) {
                    // Jika gagal cari username, lanjut coba login sbg email langsung (fallback)
                    console.log("Pencarian username gagal, mencoba sebagai email...");
                }
            }

            // 2. EKSEKUSI LOGIN AMAN
            try {
                await account.createEmailPasswordSession(inputId, pass);
            } catch (authError) {
                // *** SOLUSI ERROR "SESSION ACTIVE" ***
                // Jika errornya adalah "session active" (kode 401 atau pesan teks), 
                // anggap user SUDAH LOGIN dan biarkan lolos.
                if (authError.code === 401 || authError.message.includes('session is active')) {
                    console.log("Sesi aktif terdeteksi, melanjutkan ke dashboard...");
                } else {
                    throw authError; // Jika error lain (password salah), lempar error.
                }
            }
            
            // 3. Ambil Data User & Pindah ke Dashboard
            const user = await account.get();
            
            // Log ke Excel (Opsional, dibungkus try-catch agar tidak memblokir login)
            try {
            ...
            toggleLoading(false);
            alert("Login Gagal: " + (error.message || "Periksa koneksi atau password Anda."));
        }
    });
}
...
if (el('signupForm')) {
    el('signupForm').addEventListener('submit', async (e) => {
        ...
        if (pass !== verify) return alert("Password konfirmasi tidak sama!");
...
function initLogout() {
    ...
    if (btn) {
        // Teknik Clone Node untuk menghapus listener lama yang menumpuk
...
// ======================================================
// 4. MANAJEMEN SESI & NAVIGASI HALAMAN
// ======================================================
async function checkSession() {
    // Jangan tampilkan loading jika ini hanya refresh halaman di posisi login
    // Tapi tampilkan jika transisi login -> dashboard
    const isLoginScreen = !el('loginPage').classList.contains('hidden');
    if(isLoginScreen) toggleLoading(true, "Memuat Data...");

    try {
        currentUser = await account.get();
        // Jika berhasil get user, langsung ke Dashboard
...
// ======================================================
// 5. FITUR: SEARCH, MENU, STORAGE, DLL (TETAP ADA)
// ======================================================
...
// ... (Bagian Kontrol Klik Kanan - Anti Bentrok) ...
