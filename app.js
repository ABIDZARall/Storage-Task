...
const SHEETDB_API = 'https://sheetdb.io/api/v1/v9e5uhfox3nbi'; // Logger Excel
...
// Helper: Ambil Element dengan Cepat
...
document.addEventListener('DOMContentLoaded', () => {
    checkSession();         // Cek Login
    initDragAndDrop();      // Drag file support
    initLogout();           // Tombol logout listener
    initSearchBar();        // Search listener
    initAllContextMenus();  // Klik kanan listener
});

// ======================================================
// 3. LOGIKA OTENTIKASI (SOLUSI ERROR LOGIN)
// ======================================================
...
if (el('loginForm')) {
    el('loginForm').addEventListener('submit', async (e) => {
        ...
        try {
            // 1. Cek Username (Jika input bukan email)
            if (!inputId.includes('@')) {
                try {
                    ...
                    if (res.total > 0) {
                        inputId = res.documents[0].email; // Dapatkan email dari username
                    } else {
                        throw new Error("Username tidak ditemukan.");
                    }
                } catch(err) { console.log("Cari username gagal, mencoba sebagai email..."); }
            }
            ...
            try {
                ...
                // FIX: Jika session active, abaikan error dan lanjut login
                if (authError.code === 401 || authError.message.includes('session is active')) {
                    console.log("Sesi aktif, melanjutkan...");
                } else {
                    throw authError; // Error lain (password salah) tetap dilempar
                }
            }
            
            // 3. Log Activity & Pindah Halaman
            ...
            checkSession(); // Masuk Dashboard

        } catch (error) { 
            toggleLoading(false);
            alert("Login Gagal: " + (error.message || "Periksa koneksi/password."));
        }
    });
}
...
async function checkSession() {
    // Tampilkan loading hanya jika di halaman login
