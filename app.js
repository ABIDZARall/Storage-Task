...
function initAllContextMenus() {
    ...
    // FUNGSI PENTING: Tutup semua menu yang terbuka
    const closeAll = () => {
    ...
    // A. LOGIKA TOMBOL NEW (Klik Kiri & Kanan)
    if (newBtn) {
        const newBtnClean = newBtn.cloneNode(true); // Hapus listener lama
        ...
        newBtnClean.oncontextmenu = (e) => { e.preventDefault(); toggleNewMenu(e); };
    }

    // B. LOGIKA SIDEBAR "DRIVE SAYA" (Klik Kanan)
    if (navDrive) {
    ...
    // C. LOGIKA AREA KOSONG (Klik Kanan Global)
    if (mainArea) {
        mainArea.oncontextmenu = (e) => {
            // Jika klik pada file, jangan buka menu global
    ...
    // D. TUTUP MENU SAAT KLIK DI MANA SAJA
